...
export function CancelledServiceCard({
  entry,
  formattedTotal,
}: CancelledServiceCardProps) {
  return (
    <div className="flex flex-col sm:flex-row gap-4 rounded-2xl border border-border bg-background p-5 shadow-xs">
      {entry.imageUrl && entry.imageUrl.trim() !== "" ? (
        <img
          ...
          className="h-20 w-20 rounded-xl object-cover shrink-0 grayscale-30"
        />
      ) : (
      ...
      <div className="flex flex-1 flex-col justify-between gap-2">
        <div>
          ...
          <h2 className="text-base font-bold text-foreground mt-0.5">{entry.serviceName}</h2>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground pt-1 border-t border-border/60">
          <div className="flex items-center gap-1.5">
            ...
            <span>
              Data agendada: {entry.date} às {entry.time ?? "14:00"}
            </span>
          </div>
